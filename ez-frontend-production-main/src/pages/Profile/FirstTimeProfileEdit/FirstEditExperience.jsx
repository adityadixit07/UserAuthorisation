...
function FirstEditExperience({ user, isProfileUpdating }) {
        ...
        <>
            ...
            <div>
                <div className='flex gap-2 items-center text-base font-semibold'>
                    <h2 className='text-3xl p-4 w-full bg-gray-200 text-black font-bold rounded'>Add Work Experience</h2>
                </div>

                {user?.about?.experience.length === 0 && (
                    <FirstTimeInput
                ...
                <div className="w-full mt-4">
                    {user?.about?.experience.length > 0 && (
                        user?.about?.experience.map((item, i) => (
                            <ExperienceComponent
                                key={i}
                                item={item}
                                isProfileUpdating={isProfileUpdating}
...
const ExperienceComponent = ({ item, isProfileUpdating }) => {
...
const ExperienceInput = ({ id, isProfileUpdating, length, removeInputField }) => {
    const { handleExperienceAPI } = submitHook();
    const { fetchedCompanies, companies, fetchedJob, jobRoles, fetchedSkills, jobSkills } = useSelector(state => state.ezKaro);
...
export default FirstEditExperience