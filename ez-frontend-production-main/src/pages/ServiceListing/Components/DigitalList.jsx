            ...
            {digitalData && digitalData?.map((product, index) => (
                <div key={index} className="border border-[rgb(192,197,197)] w-full md:w-[347.8px] border-t-[12px] border-t-green-400 rounded-xl flex flex-col items-start justify-between p-3" >
                    ...
                    <div className="py-4 px-2 mt-1 border rounded border-[rgb(247,247,247)]">
                        <h2 className="font-bold">{product?.itemTitle}</h2>
                    </div>
                    ...
                    {product?.tags?.length > 0 && (
                        <div className="mt-4 flex flex-row gap-2 items-center">
                            ...
                            {product?.tags?.map((tags, index) => (
                                <p key={index} className="bg-[#B5B5B5] text-white font-bold text-[15px] py-1 px-2 mr-1 rounded-2xl">
                                    {tags.tag}
                                </p>
                            ))}
                        </div>
                    ...
                    <div className="w-full flex justify-between items-center">
                        <Link to={`checkout/${product._id}`} className="flex gap-3 items-center bg-green-500 rounded-lg p-2 font-semibold text-white">
                            Buy Product <AiFillRightCircle className="fill-white" />
                        </Link>
